...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (pathParts[0] === 'chat' && pathParts[1] === 'messages') {
      const user = await authenticate(req);
      if (req.method === 'GET') {
        return await handleGetChatMessages(req, res, user);
      }
      if (req.method === 'POST') {
        return await handlePostChatMessage(req, res, user);
      }
    }
...
async function handleDailyReportCron(req, res) {
  try {
    ...
    if (!req.headers['x-vercel-cron']) {
      return res.status(401).json({ message: 'Unauthorized - This endpoint can only be triggered by Vercel Cron' });
    }
...
async function handleGetChatMessages(req, res, user) {
  try {
    const { db } = await initializeFirebase();
...
async function handlePostChatMessage(req, res, user) {
  try {
    const { db } = await initializeFirebase();
    ...
    if (isFromUser) {
      ...
      const [productsSnapshot, categoriesSnapshot, ordersSnapshot] = await Promise.all([
        db.collection('products').where('userId', '==', user.uid).get(),
      ...
      const context = {
        ...
        categories: categories.map(c => ({
          name: c.name,
          productCount: products.filter(p => p.category === c.name || p.categoryId === c.id).length
        })),
      ...
      const aiResponse = generateChatbotResponse(message, context);
...
function generateChatbotResponse(query, context) {
